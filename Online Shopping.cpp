...
//ll cost[1000][1000],dis[1000][1000],dp[1000][1000];
...
int main()
{
    ...
    while(t--)
    {
        ll n,m;
        cin>>n>>m;
        ll cost[n+5][m+5],dis[n+5][m+5],dp[n+5][m+5];
      ...
      // for(i=0;i<m;i++) dp[0][i]=mn;
        ...
        for(i=1;i<n;i++)
        {
            ...
            for(j=0;j<m;j++)
            {
                dp[i][j]=min(dp[i-1][j]+max(0ll,cost[i][j]-dis[i-1][j]),mn+cost[i][j]);
                mn1=min(mn1,dp[i][j]);
